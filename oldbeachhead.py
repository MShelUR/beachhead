...
__copyright__ = 'Copyright 2017, 2022, University of Richmond and George Flanagin'
__credits__ = None
__version__ = '1.1'
...
class Beachhead(cmd.Cmd):
    ...
    def preloop(self) -> None:
        ...
        if not f:
            linuxutils.tombstone('You do not seem to have an ssh config file. This program')
            linuxutils.tombstone('may not be very useful.')
        ...
        try:
            ...
            try:
                ...
                linuxutils.tombstone('Using config info read from {}'.format(self.cfg_file))

            except Exception as e:
                linuxutils.tombstone(str(e))
                linuxutils.tombstone('{} failed to compile.'.format(self.cfg_file))

        except Exception as e:
            linuxutils.tombstone(e)
            linuxutils.tombstone('Something really bad happened.')
            raise

        else:
            linuxutils.tombstone('No config file found.')

    def default(self, data:str="") -> None:
        linuxutils.tombstone(red('unknown command {}'.format(data)))
        self.do_help(data)
    ...
    *********************************************************************************** """
    ...
    def do_close(self, data="") -> None:
        ...
        else: linuxutils.tombstone(blue('nothing to do'))
    ...
    def do_debug(self, data:str="") -> None:
        ...
        """
        if not len(data):
            linuxutils.tombstone(blue('debug level is {}'.format(self.hop.debug_level())))
            return
        ...
        if data not in logging_levels.keys() and data not in logging_levels.values():
            linuxutils.tombstone(red('not sure what this level means: {}'.format(data)))
            return
    ...
    def do_do(self, data:str="") -> None:
        ...
        try:
            linuxutils.tombstone(blue('attempting remote command {}'.format(data)))
            in_, out_, err_ = self.hop.channel.exec_command(data)

        except KeyboardInterrupt as e:
            linuxutils.tombstone(blue('aborting. Control-C pressed.'))

        except Exception as e:
            linuxutils.tombstone(red(str(e)))

        else:
            out_.channel.recv_exit_status();
            linuxutils.tombstone(blue(out_.readlines()))
    ...
    def do_error(self, data:str="") -> None:
        ...
        """
        linuxutils.tombstone(blue(self.hop.error))
    ...
    def do_get(self, data:str="") -> None:
        ...
        if not self.hop.sftp:
            linuxutils.tombstone(red('sftp channel is not open.'))
            return
        ...
        stop_time = time.time()
        if OK: linuxutils.tombstone('success')
        else: linuxutils.tombstone('failure {}'.format(self.hop.error_msg()))
        linuxutils.tombstone('elapsed time: {}'.format(elapsed_time(stop_time, start_time)))
    ...
    def do_hosts(self, data:str="") -> None:
        ...
        """
        linuxutils.tombstone("\n"+blue("\n".join(sorted(list(netutils.get_ssh_host_info('all'))))))
    ...
    def do_open(self, data:str="") -> None:
        ...
        if f: 
            ...
            linuxutils.tombstone(red('no operation named {}'.format(data)))
    ...
    def do_probe(self, data:str="") -> None:
        ...
        if 'all' in hostnames:
            hostnames = sorted(list(netutils.get_ssh_host_info('all')))
            hostnames.remove('*')
        ...
        try:
            ...
            for _ in hostnames:
                linuxutils.tombstone('probing {}'.format(_))
            ...
            linuxutils.tombstone("Written to logfile as transaction ID {}".format(transaction_id))
    ...
    def do_put(self, data:str="") -> None:
        ...
        if not data:
            linuxutils.tombstone(red('you have to send something ...'))
            self.do_help('put')
            return

        files = glob.glob(data)
        if not files:
            linuxutils.tombstone(red('no file[s] named {}'.format(data)))
            return
    ...
    def do_send(self, data:str="") -> None:
        ...
        if not self.hop.channel: 
            linuxutils.tombstone(red('channel not open.'))
            self.do_help('send')
            return
        ...
        try:
            ...
            linuxutils.tombstone(blue('sent {} bytes.'.format(i)))
    ...
    def do_setpass(self, data:str="") -> None:
        ...
        elif not data: linuxutils.tombstone(blue('password is set to {}'.format(self.hop.password)))
    ...
    def do_setsockdomain(self, data:str="") -> None:
        ...
        if not data: linuxutils.tombstone(blue('socket domain is {}'.format(self.hop.sock_domain))); return
        data = data.strip().lower()

        if data == 'af_inet': self.hop.sock_domain = socket.AF_INET
        elif data == 'af_unix': self.hop.sock_domain = socket.AF_UNIX
        else: linuxutils.tombstone(blue('unknown socket domain: {}'.format(data)))
    ...
    def do_setsocktype(self, data:str="") -> None:
        ...
        if not data: linuxutils.tombstone('socket type is {}'.format(self.hop.sock_type)); return

        try:
            self.hop.sock_type = sock_types[data.strip().lower()]                

        except:
            linuxutils.tombstone(blue('unknown socket type: {}'.format(data)))
    ...
    def do_settimeout(self, data:str="") -> None:
        ...
        """
        if not data: 
            linuxutils.tombstone('timeouts (tcp, auth, banner): ({}, {}, {})'.format(
                self.hop.tcp_timeout, self.hop.auth_timeout, self.hop.banner_timeout))
            return

        data = data.strip().split()
        if len(data) < 2:
            linuxutils.tombstone(red('missing timeout value.'))
        ...
        try:
            ...
            linuxutils.tombstone(red('no timeout value for ' + data[0]))
        except ValueError as e:
            linuxutils.tombstone(red('bad value for timeout: {}' + data[1]))
        else:
            self.do_settimeout()
    ...
    def do_status(self, data:str="") -> None:
        ...
        linuxutils.tombstone(blue("debug level: {}".format(self.hop.debug_level())))
        if not self.hop.sock: linuxutils.tombstone('not connected.'); return

        linuxutils.tombstone(blue("local end:     {}".format(self.hop.sock.getsockname())))
        linuxutils.tombstone(blue("remote end:    {}".format(self.hop.sock.getpeername())))
        linuxutils.tombstone(blue("type/domain:   {} / {}".format(self.hop.sock_type, self.hop.sock_domain)))
        linuxutils.tombstone(blue("ssh session:   {}".format(self.hop.client)))
        linuxutils.tombstone(blue("transport:     {}".format(self.hop.transport)))
        linuxutils.tombstone(blue("sftp layer:    {}".format(self.hop.sftp)))
        linuxutils.tombstone(blue("channel:       {}".format(self.hop.channel)))
        try:
            banner=self.hop.transport.get_banner().decode('utf-8')
        except:
            banner="no banner found"
        linuxutils.tombstone(blue("banner:        {}".format(banner)))
        linuxutils.tombstone(blue("*** security info: ***"))
        for _ in self.hop.security.keys():
            linuxutils.tombstone(blue("{} : {}").format(_,self.hop.security.get(_, None)))
    ...
    def do_save(self, data:str="") -> None:
        ...
        try:
            with open(data, 'w') as f:
                ...
                linuxutils.tombstone('Duplicate config file written to {}'.format(data))
        except:
            pass
        ...
        if new_sec_info == {}:
            linuxutils.tombstone('No active connection / no data to update.')
            return

        if old_sec_info == new_sec_info: 
            linuxutils.tombstone('Update not required for {}'.format(self.hop.remote_host))
            return

        self.cfg[self.hop.remote_host] = new_sec_info
        with open(self.cfg_file, 'w') as f:
            json.dump(self.cfg, f, sort_keys=True, indent=4)
            linuxutils.tombstone('Update successful. Written to {}'.format(self.cfg_file))
    ...
    *********************************************************************************** """
    ...
    def _do_channel(self, data:list=[]) -> None:
        ...
        if data not in channel_types.keys() and data not in channel_types.values():
            linuxutils.tombstone(blue('unknown channel type: {}'.format(data)))
            return

        linuxutils.tombstone(blue('attempting to create a channel of type {}'.format(data)))

        start_time = time.time()
        OK = self.hop.transport.open_channel(data)
        stop_time = time.time()

        if OK: linuxutils.tombstone(blue('success'))
        else: linuxutils.tombstone(red('failed ' + self.hop.error_msg()))

        linuxutils.tombstone(blue('elapsed time: {}'.format(elapsed_time(start_time, stop_time))))


    def _do_version(self) -> None:
        linuxutils.tombstone("This is the only version you will ever need.")
        linuxutils.tombstone("What difference does it make?")
        pass
    ...
    def _do_session(self, data:list=[]) -> None:
        ...
        if OK: linuxutils.tombstone(blue('ssh session established.'))
        else: linuxutils.tombstone(red('failed '+self.hop.error_msg()))
    ...
    def _do_sftp(self, data:list=[]) -> None:
        ...
        if not self.hop.transport:
            linuxutils.tombstone(red('Transport layer is not open. You must create it first.'))
            return

        linuxutils.tombstone(blue('creating sftp client.'))
        start_time = time.time()
        OK = self.hop.open_sftp()
        stop_time = time.time()

        if OK: linuxutils.tombstone(blue('success'))
        else: linuxutils.tombstone(red('failure '+self.hop.error_msg()))
    ...
    def _do_socket(self, data:list=[]) -> None:
        ...
        if len(data) < 1: 
            linuxutils.tombstone('nothing to do.')
            return
        ...
        stop_time = time.time()
        if OK: 
            linuxutils.tombstone(blue('connected.'))
        else: 
            linuxutils.tombstone(self.hop.error_msg())     
            return     
    ...
    def _do_transport(self, data:str="") -> None:
        ...
        if not self.hop.client:
            linuxutils.tombstone('You must create an ssh session before you can create a transport layer atop it.')
            return

        linuxutils.tombstone(blue('attempting to create a transport layer'))
        start_time = time.time()
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            linuxutils.tombstone("Exiting via control-C.")
            sys.exit(os.EX_OK)
